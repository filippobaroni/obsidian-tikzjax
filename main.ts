...
export default class TikzjaxPlugin extends Plugin {
	...
	async onload() {
		...
		if (this.settings.preamblePath) {
			try {
				...
				console.error(`Error loading preamble file: ${error}`);
				this.preamble = "";
			}
		} else {
			this.preamble = "";
	...
	registerTikzCodeBlock() {
		this.registerMarkdownCodeBlockProcessor("tikz", (source, el, ctx) => {
			...
			const completeSource = `${this.preamble}\n${this.settings.prefixCode}\n${source}\n${this.settings.suffixCode}`;

			script.setText(this.tidyTikzSource(completeSource));
		});
	}
